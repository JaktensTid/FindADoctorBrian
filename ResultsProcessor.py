...
class DoctorPageProcessor():
    ...
    xpath_div_where_content = '//div[@class="content"]'
    ...
    def run(self):
        ...
        with open(self.corresponding_zip +'.csv', 'w', newline='', encoding="utf-8") as csvfile:
    ...
    def get_corresponding_records(self):
        ...
        for d_link in self.doctor_page_links:
            ...
            debug_string = ("Page: {0:20}, Link: {1:20}, Title: {2:30}, Zip: {3:5}, Doctor: {4:20}").\
                format(d_link, link, title, self.corresponding_zip,doctor)
            print(debug_string)
